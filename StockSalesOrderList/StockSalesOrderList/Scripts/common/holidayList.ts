...
module holiday {
    ...
    export interface ItemHoliday {
        type: number;
        title: string;
    }

    export class Holiday {
        holidayList: Array<ItemHoliday>;
        private consts = {
        ...
        private setDayCountsInMonth(date: Date, count: number, day: number): void {
            var days: number = day - date.getDay() + 1;
            days += days < 1 ? count * 7 : (count - 1) * 7;
            date.setDate(days);
        }

        private setSyunbun(date: Date, year: number): void {
            var surplus: number = year % 4;
            var day: number = 20;
        ...
        private setSyuubun(date: Date, year: number): void {
            var surplus: number = year % 4;
            var day: number = 23;
        ...
        private setFurikae(date: Date): void {
        ...
        private setKokumin(date: Date): void {
        ...
        private setHoliday(date: Date, year: any, month: number, dateVal: any, name: string): void {
        ...
        private setObject(date: Date, name: Object): void {
            this.holidayList[this.format(date)] = name;
        }

        private inObject(date: Date): boolean {
            return this.holidayList.hasOwnProperty(this.format(date));
        }

        public getHoliday(year: any): Array<ItemHoliday> {
            this.holidayList = [];
            ...
            for (i = 0, len = this.holidays.length; i < len; i++) {
                if (this.holidays[i][0] <= year && year <= this.holidays[i][1]) {
                    this.setHoliday(
                        date,                           // Dateオブジェクト
                        year,                           // 年
                        <number>this.holidays[i][2] - 1,// 月（日付のセット用に「-1」）
                        this.holidays[i][3],            // 日 
                        <string>this.holidays[i][4]     // 祝祭日名
                    );
                }
            }

            var keys: Array<string> = Object.keys(this.holidayList).sort();

            for (i = 0, len = keys.length; i < len; i++) {
                var parse: number = Date.parse(keys[i] + " 00:00:00");
                date.setTime(parse);
